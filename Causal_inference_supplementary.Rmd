...
* Birth order: birth order refers to the order of birth with the same mother in the family. In the paper we categorized it into 2 characters: 1st born and >=2nd born, because we only included any birth after 2010 for the breastfeeding information accuracy, and the time is after full implemented child policy, so we only have 4 births as 3rd birth and we thus included them as >=2nd birth. 

* Child sex:the gender status of the child. We have two categories for this variable, daughter/female; son/male. 

* Mother age:the age of mother at the time of giving birth. We categorized in to 5 years age group:  <20, 20-25, > 25 years

* Mother cohort:this variables refers to the year of mother birth.  We categorized it into 10 years cohort:  <=1980,1980-1989 to >=1990
...
col_hh <- ifelse(household == 1, "lightgreen", ifelse(household == 2, "indianred", ifelse(household == 3, "goldenrod", "lightblue")))
 ...
 * Generate simulated duration from Poisson distribution with different causal models *(will experiment with censoring soon to match real data)*
...
residence_A <- coxph(Surv(duration_A, status_sim)~as.factor(household))
residence_B <- coxph(Surv(duration_B, status_sim)~as.factor(household))
residence_C <- coxph(Surv(duration_C, status_sim)~as.factor(household))
residence_D <- coxph(Surv(duration_D, status_sim)~as.factor(household))
